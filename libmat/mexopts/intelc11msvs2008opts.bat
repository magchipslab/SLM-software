@echo off
rem INTELC11MSVS2008OPTS.BAT
rem
rem    Compile and link options used for building MEX-files using the
rem    Intel® C++ 11.1 compiler with the Microsoft® Visual Studio® 2008
rem    SP1 Professional Edition linker
rem
rem StorageVersion: 1.0
rem C++keyFileName: INTELC11MSVS2008OPTS.BAT
rem C++keyName: Intel C++
rem C++keyManufacturer: Intel
rem C++keyVersion: 11.1
rem C++keyLanguage: C++
rem
rem    $Revision: 1.1.6.1 $  $Date: 2009/08/14 03:21:34 $
rem
rem ********************************************************************
...
set PATH=%ICPP_COMPILER11%\Bin\ia32;%VCINSTALLDIR%\BIN\;%VSINSTALLDIR%\VC\bin;%LINKERDIR%\bin;%VSINSTALLDIR%\Common7\IDE;%VSINSTALLDIR%\Common7\Tools;%VSINSTALLDIR%\Common7\Tools\bin;%VCINSTALLDIR%\VCPackages;%MATLAB_BIN%;%PATH%
set INCLUDE=%ICPP_COMPILER11%\Include;%VCINSTALLDIR%\ATLMFC\INCLUDE;%VCINSTALLDIR%\INCLUDE;%LINKERDIR%\include;%INCLUDE%
set LIB=%ICPP_COMPILER11%\Lib\ia32;%VCINSTALLDIR%\ATLMFC\LIB;%VCINSTALLDIR%\LIB;%LINKERDIR%\lib;%VSINSTALLDIR%\SDK\v2.0\lib;%MATLAB%\extern\lib\win32;%LIB%
